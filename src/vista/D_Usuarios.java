...
public class D_Usuarios extends javax.swing.JInternalFrame {
    
    private TablaUsuario modelUsuario = new TablaUsuario();
    private UsuarioJpaController usuarioJpa;
    private Persistencia pst;
    private Usuario usuario;
    private ListSelectionModel listaUsuarioselect;
    private boolean seleccionar = false;
    
    public D_Usuarios(Persistencia pst) {
        
        initComponents();
        
        this.pst = pst;
        usuarioJpa = new UsuarioJpaController(pst.getFactory());
        modelUsuario.setFilaUser(usuarioJpa.findUsuarioEntities());//Sentencia que obtiene los datos de la BD

        tablaUsuarios.setModel(modelUsuario);
    }
    ...
    private void initComponents() {
        ...
        listaUsuarioselect=tablaUsuarios.getSelectionModel();
        listaUsuarioselect.addListSelectionListener(new ListSelectionListener(){
            public void valueChanged(ListSelectionEvent e){
                if(!e.getValueIsAdjusting()){
                    tbPersonaLineaSeleccionada(tablaUsuarios);
                }
            }
        ...
        jPanel2.add(txtContraseña, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 140, 180, -1));

        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/iconoBrazoalcancia.png"))); // NOI18N
        jPanel2.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 160, 250, 250));

        txtNombreDeUsuario.setBorder(null);
        jPanel2.add(txtNombreDeUsuario, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 80, 180, -1));
    ...
    public void tbPersonaLineaSeleccionada(JTable tabla) {
        if (tabla.getSelectedRow() != -1) {
            seleccionar = true;
            controlBotones();
            usuario = modelUsuario.getFilaUser().get(tabla.getSelectedRow());
            
            txtNombreDeUsuario.setText(usuario.getUsuario());
...
//            btnLimpiar.setEnabled(true);
    ...
    public void limpiarDatos() {
        ...
        tablaUsuarios.getSelectionModel().clearSelection();//Limpia la seleccion de la tabla o la linea resaltada con la flecha

        usuario = null; // Despues de obtener la linea seleccionada borra los campos de los txt
        controlBotones();
    }
    
    public Usuario validarCampos() {
        if (!txtNombreDeUsuario.getText().equals("")) {
            usuario.setUsuario(txtNombreDeUsuario.getText());
...
//            persona = new Persona(); //Despues de obtener y limpiar los campos vuelve a designar a una persona como vacio
            usuarioJpa.create(validarCampos());
            modelUsuario.agregar(usuario);
            limpiarDatos();
