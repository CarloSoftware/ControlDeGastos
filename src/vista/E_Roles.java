...
import modeloTablas.TablaRol;
import persistencia.Persistencia;
import persistencia.Recursos;


public class E_Roles extends javax.swing.JInternalFrame {
    
    private TablaRol modelRol = new TablaRol();
    private RolJpaController rolJpa;
    private Persistencia pst;
    private Rol rol;
    private ListSelectionModel listaRolselect;
    private boolean seleccionar = false;
    
    public E_Roles(Persistencia pst) {
        
        initComponents();
        
        this.pst = pst;
        rolJpa = new RolJpaController(pst.getFactory());
        modelRol.setFilaRol(rolJpa.findRolEntities());//Sentencia que obtiene los datos de la BD

        tablaRol.setModel(modelRol);
    }
    ...
    private void initComponents() {
        ...
        txtNombreDeRol = new javax.swing.JTextField();
        ...
        tablaRol.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        listaRolselect=tablaRol.getSelectionModel();
        listaRolselect.addListSelectionListener(new ListSelectionListener(){
            public void valueChanged(ListSelectionEvent e){
                if(!e.getValueIsAdjusting()){
                    tbRolLineaSeleccionada(tablaRol);
                }
            }
        ...
        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/iconoConfiguracion.png"))); // NOI18N
        jPanel2.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 160, 250, 250));

        txtNombreDeRol.setBorder(null);
        jPanel2.add(txtNombreDeRol, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 80, 180, -1));

        jLabel3.setFont(new java.awt.Font("Raleway", 0, 14)); // NOI18N
        jLabel3.setText("NOMBRE DE ROL:");
    ...
    private javax.swing.JTextField txtNombreDeRol;
    // End of variables declaration//GEN-END:variables

    public void tbRolLineaSeleccionada(JTable tabla) {
        if (tabla.getSelectedRow() != -1) {
            seleccionar = true;
            controlBotones();
            rol = modelRol.getFilaRol().get(tabla.getSelectedRow());
           
            txtNombreDeRol.setText(rol.getRol());
            txtDescripción.setText(rol.getDescripcion());
...
//            btnLimpiar.setEnabled(true);
    ...
    public void limpiarDatos() {
        ...
        txtNombreDeRol.setText("");
        txtDescripción.setText("");
        
        seleccionar = false;
        
        tablaRol.getSelectionModel().clearSelection();//Limpia la seleccion de la tabla o la linea resaltada con la flecha

        rol = null; // Despues de obtener la linea seleccionada borra los campos de los txt
        controlBotones();
    }
    
    public Rol validarCampos() {
        if (!txtNombreDeRol.getText().equals("")) {
            rol.setRol(txtNombreDeRol.getText());
            
            if (!txtDescripción.getText().equals("")) {
                rol.setRol(txtDescripción.getText());
            } else {
...
//            persona = new Persona(); //Despues de obtener y limpiar los campos vuelve a designar a una persona como vacio
            rolJpa.create(validarCampos());
            modelRol.agregar(rol);
            limpiarDatos();
    ...
    public void editarRol() {
        if (validarCampos() != null) {
            rol.setRol(txtNombreDeRol.getText());
            rol.setDescripcion(txtDescripción.getText());
